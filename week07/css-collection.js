...
var result = [];
for (let list of document.getElementById("container").children){
    if(list.getAttribute('data-tag').match(/css/)) {
        result.push({
            name: list.children[1].innerText,
            url: list.children[1].children[0].href
        });
    }
}
